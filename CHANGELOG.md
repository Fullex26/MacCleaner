...
### Fixed
- **`storage-insights` still hung forever, and 2.17.2's fix could never have caught it.** That release assumed a directory only blocks `readdir` when it is an evicted iCloud folder carrying `SF_DATALESS`. Not so: `~/Library/Containers/com.apple.dt.ExternalViewService/Data` on a real developer Mac is a plain user-owned directory — mode `0700`, `st_flags == 0`, not dataless, not iCloud, just an Xcode app container — and a bare `os.listdir()` on it never returns. A real scan sat in `os_scandir` on it for **12 minutes at 0% CPU**, and the CLI has no SIGKILL backstop, so it hung indefinitely. The 120-second budget could not help, because it was only checked *between* directories and a readdir already blocked in the kernel cannot be interrupted from Python. The walk itself is now what gets bounded: it runs in a daemon thread the caller joins with the budget, and a wedged thread is abandoned rather than waited on. Verified on the machine that hung — 12+ minutes became a clean 120.2 s return with `truncated: true` and 47 real results. Enumerating classes of blocking directory is a losing game; the bound is the fix.
