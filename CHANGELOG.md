...
- **Dashboard "Large Files" section** (app) — a new panel below the existing target groups, populated from `storage-insights --json`, listing each file's size and a **Reveal in Finder** action (`NSWorkspace.shared.activateFileViewerSelectingURLs`). Purely informational — the app never offers to delete a large file from this section, matching the CLI's read-only design. Degrades to an explanatory empty/error state on an older bundled engine that predates this subcommand (see `AGENTS.md` engine/app version-skew notes).
