...
`scheduler.sh weekly|monthly` installs two launchd agents (`com.fullex.maccleaner.clean` — `StartCalendarInterval`, runs `clean --yes --notify`; `com.fullex.maccleaner.diskwatch` — `StartInterval` 3600s, runs `disk-check`), replacing the old cron-based scheduling. launchd catches up on a missed calendar run after sleep/wake, unlike cron. `status` is read-only; `remove` unloads both agents. An existing cron line is auto-migrated to the equivalent launchd schedule (same cadence) the first time `weekly`/`monthly` runs, and removed as part of that migration; `doctor`'s Schedule check reports installed launchd agents and flags a leftover legacy cron entry.
