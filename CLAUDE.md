...
### Tests
```bash
python3 -m unittest discover -s tests    # 249 tests, stdlib only, no deps
```
...
- `collect_targets(config, all_categories=False, categories=None, target_ids=None)` — new in 2.5.0, extended in 2.6.0; what `scan`/`clean`/`clean --dry-run` actually call instead of `get_targets()` directly. Appends `tmp_to_targets(scan_tmp_artifacts(config))` when `tmp` is enabled and `scan_simulator_targets(config)` when `simulators` is enabled, on top of the static list. `categories`/`target_ids` are v2.6 SELECTION HINTS from the CLI invocation (`--category`/`--targets`): when a hint proves a dynamic scanner's output can't possibly be selected, that scanner is skipped entirely — e.g. `clean --targets npm-cache` no longer pays a `simctl` call or a `/private/tmp` walk it was never going to use. **Hints only ever narrow, never widen** — `enabled_categories` still gates as before, and `categories=None, target_ids=None` (the default) behaves exactly like pre-2.6. `categories` (the subcommand, not this parameter) deliberately keeps calling `get_targets()` alone — the per-directory `tmp-<slug>` IDs are unstable across runs and the completions live-ID pipeline (`categories --json`) must never surface them. `scan_tmp_artifacts()` walks only the top level of the tmp root (`/private/tmp` by default, `MACCLEANER_TMP_ROOT` overridable), classifying a directory as junk purely by *content* (Xcode DerivedData shape, or a `.git` clone with a manifest + build-artifact dir) — never by name — and skipping symlinks, other-owner dirs, active `claude-*` session scratch dirs, and anything younger than `tmp_min_age_days`. `scan_simulator_targets()` shells out to `xcrun simctl list devices -j` / `runtime list -j` and produces up to two fixed, conditional IDs (`simulator-stale-devices`, `simulator-unused-runtimes`) whose `cmd` strings are built only from simctl UDIDs/identifiers that pass a strict regex first. Both dynamic families are always `safe: False` (review-only). The home-only rule in `delete_target`/`_safe_to_delete` has exactly one carve-out for this: a `tmp_scan`-marked target (set only by `tmp_to_targets`) may delete a path outside `$HOME` if — and only if — that path resolves to a direct child of the tmp scan root; nothing else, and no config key widens it.
