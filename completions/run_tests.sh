...
eq "subcommands"          "12" "$(B 'maccleaner ' | grep -cv '^--')"
eq "prefix filter 'sc'"   "scan schedule" "$(B 'maccleaner sc' | tr '\n' ' ' | sed 's/ $//')"
eq "schedule actions"     "status weekly monthly off run" "$(B 'maccleaner schedule ' | grep -v '^--' | tr '\n' ' ' | sed 's/ $//')"
eq "schedule after action" "--json --help" "$(B 'maccleaner schedule weekly ' | tr '\n' ' ' | sed 's/ $//')"
eq "clean flags"          "10"  "$(B 'maccleaner clean --' | wc -l | tr -d ' ')"
eq "targets 1st value"    "94" "$(B 'maccleaner clean --targets ' | wc -l | tr -d ' ')"
eq "targets 2nd value"    "93" "$(B 'maccleaner clean --targets npm-cache,' | wc -l | tr -d ' ')"
eq "targets 3rd value"    "92" "$(B 'maccleaner clean --targets npm-cache,pip-cache,' | wc -l | tr -d ' ')"
...
eq "alias mclean flags"    "--yes --targets --category --min-size --min-free --trash --dry-run --notify --json --help" "$(B 'mclean ' | tr '\n' ' ' | sed 's/ $//')"
...
eq "zsh subcommands"      "12" "$(Z 'maccleaner ' | wc -l | tr -d ' ')"
eq "zsh prefix 'sc'"      "scan schedule" "$(Z 'maccleaner sc' | tr '\n' ' ' | sed 's/ $//')"
eq "zsh schedule action"  "monthly off run status weekly" "$(Z 'maccleaner schedule ' | tr '\n' ' ' | sed 's/ $//')"
...
eq "zsh alias mclean flags" "--category --dry-run --help --json --min-free --min-size --notify --targets --trash --yes -h" "$(Z 'mclean ' | tr '\n' ' ' | sed 's/ $//')"
...
eq "zsh alias mclean --targets (multi-word tail)" \
   "93" "$(Z 'mclean --targets npm-cache,' | wc -l | tr -d ' ')"
eq "zsh alias mclean --yes --targets (3-word tail)" \
   "93" "$(Z 'mclean --yes --targets npm-cache,' | wc -l | tr -d ' ')"
eq "zsh non-alias parity for the same case" \
   "93" "$(Z 'maccleaner clean --targets npm-cache,' | wc -l | tr -d ' ')"
...
# The live target count has a static floor of 94 (v2.11) but the `logs`
...
ge "real engine bash: --targets count reaches the static floor" \
   "94" "$(RB 'maccleaner clean --targets ' | wc -l | tr -d ' ')"
...
ge "real engine zsh: --targets count reaches the static floor" \
   "94" "$(RZ 'maccleaner clean --targets ' | wc -l | tr -d ' ')"
