...
- **CLI** — `scan`, `clean`, `projects`, `report`, `doctor`, `config`. 94 cleanup targets across 23 categories. Interactive checklist or fully unattended.
...
23 categories, 94 targets. Enable or disable any category via `maccleaner config enable|disable <category>`; run `maccleaner categories` to list every target and its ID.
