...
```json
{
  ...
  "checks": [
    ...
    { "name": "Swap",         "status": "9.1 GB of 15.0 GB used (60.7%)", "ok": true },
    { "name": "Held-open files", "status": "1.2 GB of deleted files still held open by Slack (829.7 MB), node (391.0 MB) — the space frees itself when they exit", "ok": false }
  ]
}
```

Top-level `ok` is the logical AND of every `checks[].ok`, which is why it is `false` above. **The process still exits `0`** — see the exit-code note below.
...
`ok` is false only for genuine problems: invalid config JSON, (see Schedule below) a launchd plist present but not actually loaded, (see Schedule paths below) a scheduled agent whose interpreter or engine script no longer exists on disk, or (new in 2.8.0, see below) sustained swap pressure / a large volume of deleted-but-still-open files. Missing optional tools are informational (`ok: true`). This is `doctor`'s own summary field, not a process exit code — `main` doesn't propagate `run_doctor`'s return value, so `doctor`'s exit code stays governed by the usual 0/1/2 contract regardless of this `ok` value.
...
The `Swap` check (new in 2.8.0) is **report-only** and always present in `checks[]`. It reads `sysctl vm.swapusage` (5-second timeout) and reports used-of-total with a percentage. It flags `ok: false` only at **75% or more** of swap in use, with a `"— high; close memory-heavy apps or restart to free it"` hint. Total swap of zero reports `"not in use"` (`ok: true`). If `sysctl` is missing, exits non-zero, times out, or emits output the parser can't read, it reports `"could not determine swap usage"` (`ok: true`) — a best-effort probe never fails the doctor run. Report-only means exactly that: macOS manages the swap file itself and shrinks it as memory pressure drops, so **there is no target ID for this and no cleanup action is ever offered**; agents must not try to "clean" swap.

The `Held-open files` check (new in 2.8.0) is **report-only** and **conditional** — it appears in `checks[]` only when deleted-but-still-open files total **500 MB or more**, and when it appears it is always `ok: false`. It shells out to `lsof -nPw +c 0 +L1` (10-second timeout) and sums regular files with a zero link count, deduping by `(DEVICE, NODE)` so one deleted inode held by several processes — or by several file descriptors within one process — is counted once rather than multiplied. The status names up to **3** holding commands (each contributing 10 MB or more; if no single holder clears that floor the biggest are named anyway) with a `"+N more process"` / `"+N more processes"` tail when others remain. If `lsof` is missing, times out, or prints nothing usable, the check is simply omitted — its absence is not a health signal. This space is reclaimed by the OS the moment the holding process exits, so, like `Swap`, **it has no target ID and MacCleaner will never kill a process or delete anything on its behalf**.

Both 2.8.0 checks use hardcoded thresholds (75% swap; 500 MB held-open total, 10 MB per-process naming floor) — they are **not** configurable, and no `config.json` key affects them.
...
Oldest → newest, last N runs (`-n`, default 10). The log file keeps the last 50 runs. With no history: `{"version": "2.7.0", "runs": [], "disk_history": {...}}` (`disk_history` is present either way).
...
- **New in 2.8.0** — 3 new static targets in existing categories, all `"safe": true`, no new categories: `xcodebuildmcp-workspaces` (`xcode` — XcodeBuildMCP workspace scratch data), `chrome-optimization-model-store` and `chrome-optimization-hint-cache` (`caches` — Chrome's downloaded on-device ML models and its per-profile page-optimization hint cache; both re-fetched on demand). All additive — no existing IDs changed. This takes the static target table to 83.
